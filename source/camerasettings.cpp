...
bool CameraSettings::is_valid_type(quint32 type)
{
    switch (type) {
...
void CameraSettings::addCtrl(QGridLayout *grid, const v4l2_query_ext_ctrl &qec)
{
...
void CameraSettings::fixWidth(QGridLayout *grid)
{
...
void CameraSettings::finishGrid(QGridLayout *grid, unsigned which)
{
...
void CameraSettings::addWidget(QGridLayout *grid, QWidget *w, Qt::Alignment align)
{
...
void CameraSettings::refresh(unsigned which)
{
...
void CameraSettings::error(const QString &error)
{
    qDebug() << "Error: " << error;
}

void CameraSettings::error(int err)
{
    error(QString("Error: %1").arg(strerror(err)));
}

void CameraSettings::errorCtrl(unsigned id, int err)
{
    error(QString("Error %1: %2")
        .arg((const char *)m_ctrlMap[id].name).arg(strerror(err)));
}

void CameraSettings::errorCtrl(unsigned id, int err, const QString &v)
{
    error(QString("Error %1 (%2): %3")
        .arg((const char *)m_ctrlMap[id].name).arg(v).arg(strerror(err)));
}

void CameraSettings::errorCtrl(unsigned id, int err, long long v)
{
    error(QString("Error %1 (%2): %3")
        .arg((const char *)m_ctrlMap[id].name).arg(v).arg(strerror(err)));
}

void CameraSettings::setVal64(unsigned id, long long v)
{
    const v4l2_query_ext_ctrl &qec = m_ctrlMap[id];
...
void CameraSettings::setString(unsigned id, const QString &v)
{
    const v4l2_query_ext_ctrl &qec = m_ctrlMap[id];
    QWidget *w = m_widgetMap[qec.id];
...
void CameraSettings::setVal(unsigned id, int v)
{
    const v4l2_query_ext_ctrl &qec = m_ctrlMap[id];
...
void CameraSettings::setWhat(QWidget *w, unsigned id, long long v)
{
    const v4l2_query_ext_ctrl &qec = m_ctrlMap[id];
    QString what;
...
void CameraSettings::setWhat(QWidget *w, unsigned id, const QString &v)
{
    const v4l2_query_ext_ctrl &qec = m_ctrlMap[id];
    QString what;
    QString flags = getCtrlFlags(qec.flags);
...
QString CameraSettings::getCtrlFlags(unsigned flags)
{
    QString s;
...
void CameraSettings::setDefaults(unsigned which)
{
...
void CameraSettings::updateCtrl(unsigned id)
{
    unsigned which = V4L2_CTRL_ID2WHICH(id);
    if (which == V4L2_CID_PRIVATE_BASE)
        which = V4L2_CTRL_CLASS_USER;
...
long long CameraSettings::getVal64(unsigned id)
{
    const v4l2_query_ext_ctrl &qec = m_ctrlMap[id];
    QWidget *w = m_widgetMap[qec.id];
    long long v = 0;
...
QString CameraSettings::getString(unsigned id)
{
    const v4l2_query_ext_ctrl &qec = m_ctrlMap[id];
    QWidget *w = m_widgetMap[qec.id];
    QString v;
...
int CameraSettings::getVal(unsigned id)
{
    const v4l2_query_ext_ctrl &qec = m_ctrlMap[id];
    QWidget *w = m_widgetMap[qec.id];
    v4l2_querymenu qmenu;
